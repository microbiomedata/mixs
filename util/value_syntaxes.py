...
core_file = "../downloads/gsc_mixs6_core.tsv"
packages_file = "../downloads/gsc_mixs6.tsv"
...
vs_valcounts = id_to_vs['Value syntax'].value_counts().rename_axis('Value syntax').reset_index(name='term count')
...
vs_valcounts.to_csv("../target/vs_usage.tsv", sep="\t", index=False)

token_valcounts = usage_frame['token'].value_counts().rename_axis('token').reset_index(name='Value syntax count')

token_valcounts.to_csv("../target/vs_token_usage.tsv", sep="\t", index=False)
...
with open("../target/vs_untokenized.txt", 'w') as fp:
